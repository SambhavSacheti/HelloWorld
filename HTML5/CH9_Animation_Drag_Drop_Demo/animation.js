...
window.onload = function () {
  ...
  canvas.onmousedown = canvasMouseDownClick;
  canvas.onmouseup = canvasMouseUpClick;
  canvas.onmousemove = canvasMouseDrag;
  ...
  for (var i in balls) {
    ...
    if ((clickX > (ball.x - ball.radius)) && (clickX < (ball.x + ball.radius))) {
      if ((clickY > (ball.y - ball.radius)) && (clickY < (ball.y + ball.radius))) {   
        ball.isDragging = false;                        // On Mouse up set isDragging property to fase.
        return;
      }
    }
  }
...
function canvasMouseDownClick(e) {
  // Get the canvas click coordinates.
  var clickX = e.pageX - canvas.offsetLeft;
  var clickY = e.pageY - canvas.offsetTop;
  // Look for the clicked ball.
